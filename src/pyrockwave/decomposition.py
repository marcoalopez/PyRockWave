...
# Description: Browaeys & Chevrot (2004) symmetry-class decomposition of the  #
# elastic tensor and the associated percentage contributions.                #
#                                                                             #
...
def decompose_Cij(Cij: np.ndarray) -> dict[str, np.ndarray]:
    ...
    for symmetry_class in decomposed_elements:
        ...
        if symmetry_class != "others":
            X_total = _tensor_to_vector(Cij_copy)

            # compute the vector X on a specific symmetry subspace
            M = _orthogonal_projector(symmetry_class)
            X_symmetry_class = np.dot(M, X_total)  # X_h = M*X

            C_symmetry_class = _vector_to_tensor(X_symmetry_class)
...
def _tensor_to_vector(Cij: np.ndarray) -> np.ndarray:
    """
...
def _vector_to_tensor(X: np.ndarray) -> np.ndarray:
    """
...
def _orthogonal_projector(symmetry_class: str) -> np.ndarray:
    """
...
def calc_percentages(decomposition: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    sq_norms = {tc: np.linalg.norm(_tensor_to_vector(decomposition[tc])) ** 2
